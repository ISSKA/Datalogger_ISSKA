...
void initialize_notecard() {
  // Alimente le Notecard et configure le multiplexeur
  digitalWrite(MOSFET_NOTECARD_PIN, HIGH);
  delay(100);
  sensor.tcaselect(NOTECARD_I2C_MULTIPLEXER_CHANNEL);
  delay(100);

  // Initialise le Notecard
  notecard.begin();
  delay(400);

  // Configure le mode du Notecard et le hub
  ...
  if (req_set != NULL) {
    ...
    Serial.println("Erreur: Impossible de créer une requête hub.set.");
  }
}

bool synchronize_notecard() { 
  // Retourne true si la synchronisation avec le réseau GSM est réussie
  ...
  while (true) {
    ...
    if (SyncStatus != NULL) {
      int completed = (int)JGetNumber(SyncStatus, "completed");
      notecard.deleteResponse(SyncStatus);
      if (completed != 0) {
        Serial.println("Le Notecard est synchronisé!");
        return true;
      }
    } else {
      Serial.println("Erreur: Échec de la récupération de l'état de synchronisation.");
      return false;
    }
    if ((millis() - start_time_connection) > 240000) { // Timeout après 4 minutes
      Serial.println("Le Notecard a échoué à se synchroniser (timeout)." );
      return false;
    }
    delay(500); // Vérifie l'état toutes les 500 ms
  }
}
...
void send_data_overGSM() {
  // Lecture des données depuis la carte SD
  ...
  // Détermination de la position des dernières lignes
  ...
  while (myFile.available() && counter < nb_meas_sent_at_once) {
    for (int i = 0; i < 2 + sensor.get_nb_values(); i++) {
      ...
      if (element.length() > 0 && i == 1) {
        ...
        time_array[counter] = datetime.unixtime() - 3600; // Conversion UTC+1 -> UTC+0
  ...
  // Impression des données à envoyer
  Serial.println("Préparation des données à envoyer :");
  for (int i = 0; i < nb_meas_sent_at_once; i++) {
    Serial.print("Heure: ");
  ...
  // Crée le fichier JSON pour l'envoi
  ...
  for (int i = 0; i < sensor.get_nb_values(); i++) {
    J *sensor_array = JAddArrayToObject(data, sensor_names[i].c_str());
    for (int j = 0; j < nb_meas_sent_at_once; j++) {
      J *sample = JCreateObject();
      JAddItemToArray(sensor_array, sample);
  ...
  Serial.println("Contenu de la requête JSON :");
