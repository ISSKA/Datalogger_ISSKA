...
unsigned long NoteHubSendData()
{
  ...
  J* TemperatureWater_J = JAddArrayToObject(tempData, "Temperature_Water_(degC)");
  J* PressureWater_J = JAddArrayToObject(tempData, "Pressure_Water_(mbar)");
  J* WaterLevel_J = JAddArrayToObject(tempData, "Water_Level_(cm)");
  ...
  for (int i = 0; i < MAX_MEASUREMENTS; i++)
  {
    ...
    //Temperature Water
    sample = JCreateObject();
    JAddItemToArray(TemperatureWater_J, sample);
    //Add value of the sample
    ...
    //Pressure Water
    sample = JCreateObject();
    JAddItemToArray(PressureWater_J, sample);
    //Add value of the sample
    value = JCreateString(String(LastMeasurements[i][6], 2).c_str());
    JAddItemToObject(sample, "value", value);
    //Add timeStamp of the sample
    epoch = JCreateNumber(DatesUNIX[i]);
    JAddItemToObject(sample, "epoch", epoch);
    ...
    //Water level
    sample = JCreateObject();
    JAddItemToArray(WaterLevel_J, sample);
    //Add value of the sample
    value = JCreateString(String(LastMeasurements[i][7], 2).c_str());
    JAddItemToObject(sample, "value", value);
    //Add timeStamp of the sample
    epoch = JCreateNumber(DatesUNIX[i]);
    JAddItemToObject(sample, "epoch", epoch);
