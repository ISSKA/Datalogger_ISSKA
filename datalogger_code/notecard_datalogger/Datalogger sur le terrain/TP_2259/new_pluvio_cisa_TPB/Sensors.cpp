...
String Sensors::getFileHeader () { // Return the file header for the sensors part. This should be something like "<sensor 1 name>;<sensor 2 name>;"
  return "tempSHT;humSHT;tempBMP;pressBMP;tempPluv;presPluv;presWat;htWat;"; //change if you add sensors !!!!!!!!
}
...
String Sensors::getFileData () { // Return sensors data formated to be write to the CSV file. This should be something like "<sensor 1 value>;<sensor 2 value>;"
  ...
  sprintf(str, "%s;%s;%s;%s;%s;%s;%s;%s;", //"temp", "press", "Hum" , ... put as many %s; as there are sensors!!!!!! don't forget the last ";" !!
     String(mesures.tempSHT,1).c_str(), String(mesures.humSHT,1).c_str(), String(mesures.tempBMP,1).c_str(),String(mesures.pressBMP,0).c_str(), String(mesures.tempPluv,1).c_str(), String(mesures.presPluv,0).c_str(),String(mesures.presWat,0).c_str(),String(mesures.htWat,1).c_str());
  return str;
}
...
String Sensors::serialPrint() { //Display sensor mesures to Serial for debug purposes
  ...
  sensor_values_str = sensor_values_str + "humSHT: " + String(mesures.humSHT,1) + "\n";
  sensor_values_str = sensor_values_str + "tempBMP: " + String(mesures.tempBMP,1) + "\n";
  sensor_values_str = sensor_values_str + "pressBMP: " + String(mesures.pressBMP,0) + "\n";
  sensor_values_str = sensor_values_str + "tempPlv: " + String(mesures.tempPluv,1) + "\n";
  sensor_values_str = sensor_values_str + "presPlv: " + String(mesures.presPluv,0) + "\n";
  sensor_values_str = sensor_values_str + "presWat: " + String(mesures.presWat,0) + "\n";
  sensor_values_str = sensor_values_str + "htWat: " + String(mesures.htWat,1) + "\n";
