...
bool synchronize_notecard(){ //returns true if it was able to connect the notecard to GSM nework
  ...
  sensor.tcaselect(3); //the notecard is connected to TCA3 from the multiplexer on isska-logger v1.0 //the notecard is connected to TCA3 from the multiplexer on isska-logger v1.0
  notecard.begin();
  delay(400);
...
void send_data_overGSM(){ //send several measurements all at once over GSM
  ...
  */
  ...
  digitalWrite(MOSFET_NOTECARD_PIN, HIGH); 
  sensor.tcaselect(3); //the notecard is connected to TCA3 from the multiplexer on isska-logger v1.0
  delay(100);
  ...
  if(synchronize_notecard()){ //connect the notecard
    ...
    String sensor_names[num_sensors+1] = {"VBat","tempSHT","pressBMP","humSHT","tempBMP","ppmCO2","percentO2"}; //change this if you add more sensors!!!
