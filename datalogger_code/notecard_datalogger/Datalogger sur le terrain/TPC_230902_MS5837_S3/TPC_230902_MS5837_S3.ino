...
#define Enable5VPIN 4 //turns the 3V to 5V converter on if switched to high
#define MOSFET_SENSORS_PIN 2 //pin which controls the power of the sensors, the screen and sd card reader
#define MOSFET_NOTECARD_PIN 1 //pin which controls the power of the notecard
...
//SD card
#define SD_CS_PIN 34//Define the pin of the tinypico connected to the CS pin of the SD card reader
...
void setup(){ 
  ...
  if(first_time){ //this snipet is only executed when the microcontroller is turned on for the first time
    tinyS3.begin();
    tinyS3.setPixelPower(true); //This method turns on the Dotstar power switch, which enables the tinypico LED
    tinyS3.setPixelColor(25, 25, 25 ); //Turn the LED white while SD card is not initialised
    ...
    if (bootCount<=3){ //Turn the LED blue during first 3 measurements ( only to check that the logger works normally)
      tinyS3.setPixelPower(true);
      tinyS3.setPixelColor(0, 0, 25 );  
    }
    ...
    if(tinyS3.getBatteryVoltage()>3.5){
...
void put_usefull_values_on_display(){
  ...
  float batvolt = tinyS3.getBatteryVoltage();
...
void initialise_SD_card(){ //open SD card. Write header in the data.csv file. read values from the conf.txt file.
  ...
  if (SD_error == true){
    ...
    tinyS3.setPixelColor( 50, 0, 0 ); //set led to red
  }
  //write header in SD
  else {
...
void deep_sleep_mode(int sleeping_time){ 
  ...
  digitalWrite(MOSFET_NOTECARD_PIN, LOW);
  tinyS3.setPixelPower(false); //LED of the TinyPico //LED of the TinyPico
...
void save_data_in_SD(){ 
  ...
  if (!SD.begin(SD_CS_PIN)) { //if there is a problem with the SD card turn on display to show to the user that there is a problem with the SD card
    ...
    u8x8.print("SD Failure !");
    tinyS3.setPixelPower(true);
    tinyS3.setPixelColor( 50, 0, 0 ); //set led to red//set led to red
...
void initialize_notecard(){
 //power the notecard and switch the multiplexer to its I2c line
  ...
  //J *req_aux = notecard.newRequest("card.aux"); 
  //JAddStringToObject(req_aux,"mode","off");
  //notecard.sendRequest(req_aux);
}
...
unsigned int getGSMtime(){
  ...
  tinyS3.setPixelColor(25, 0, 25 ); //LED Purple
  ...
  if(synchronize_notecard()){
    ...
    u8x8.print("GSM Bars: ");
    u8x8.print(bars);
    tinyS3.setPixelColor(0, 50, 0 ); //LED Green
    ...
    tinyS3.setPixelColor(50, 0, 0 ); //LED Red
    digitalWrite(MOSFET_NOTECARD_PIN, LOW); //turn off the notecard
    sensor.tcaselect(0); //connect the multiplexer to another I2c line to avoid having the notecard interfer with other devices
