...
/* *******************************************************************************
Base code for datalogger with Notecard, with MS8607 and MS5803 connected at 40m  (temp, hum, press). As new functionality we can now remotely change some parameters on the conf.txt file remotely.

!!!!THIS CODE IS SPECIAL BECAUSE WE CHANGE THE I2C FREQUENCIES WHEN COMMUNICATING WITH THE PRESSUR SENSOR SINCE THE WIRE IS LONG!!!
...
RTC_DATA_ATTR int bootCount_since_change = 0;
...
void setup(){ //The setup is recalled each time the micrcontroller wakes up from a deepsleep
  ...
  if(first_time){ //sleep until rounded time, this snipet is only executed when the microcontroller is turned on for the first time
    ...
    //setup for the clock
    rtc.begin();
    rtc.disable32K(); //there is a 32kHz output from the rtc which we don't use so we desable it
    ...
    tp.DotStar_SetPixelColor(0, 0, 25 ); //led blue while measuring
...
void put_usefull_values_on_display(){
  ...
  u8x8.display(); // initialize OLED display
  u8x8.clear();
  u8x8.setCursor(0, 0);
  u8x8.print("Batt: ");
  u8x8.print(batvolt);
...
void get_external_parameter(){
  ...
  if (nb_changes >0){
    ...
    if(new_deep_sleep_time!=0 || new_MAX_MEASUREMENTS!=0){
      ...
      Serial.print("new config file: ");
      Serial.println(new_conf_file_str);
      delay(50);
      configFile.close();
      if(bootCount!=0){
        DateTime starting_time_dt = DateTime(start_date_time[0],start_date_time[1],start_date_time[2],start_date_time[3],start_date_time[4],start_date_time[5]);
...
void send_data_overGSM(){ //send several measurements all at once over GSM
  ...
  */
  ...
  if(synchronize_notecard()){ //connect the notecard
    ...
    String sensor_names[num_sensors+1] = {"VBat","tempSHT","pressBMP","humSHT","tempBMP","O2","CO2","SensorVbat"}; //change this if you add more sensors!!!
    ...
    JAddBoolToObject(req, "sync", true); //initiate sync immediately
