...
unsigned long NoteHubSendData()
{
  ...
  J* Vbatt_J = JAddArrayToObject(tempData, "Battery_Voltage_V");
  ...
  J* O2_J = JAddArrayToObject(tempData, "O2_percentage");
  J* CO2_J = JAddArrayToObject(tempData, "CO2_ppm");
  J* sensorsVbat_J = JAddArrayToObject(tempData, "Sensor_Battery_Voltage_V");
  ...
  for (int i = 0; i < MAX_MEASUREMENTS; i++)
  {
    ...
    //Add value of the sample
    value = JCreateString(String(LastMeasurements[i][6], 0).c_str());
    JAddItemToObject(sample, "value", value);
    //Add timeStamp of the sample
    epoch = JCreateNumber(DatesUNIX[i]);
    JAddItemToObject(sample, "epoch", epoch);

    //7. Sensors battery voltage
    sample = JCreateObject();
    JAddItemToArray(sensorsVbat_J, sample);
    //Add value of the sample
    value = JCreateString(String(LastMeasurements[i][7], 0).c_str());
    JAddItemToObject(sample, "value", value);
    //Add timeStamp of the sample
    epoch = JCreateNumber(DatesUNIX[i]);
    JAddItemToObject(sample, "epoch", epoch);
