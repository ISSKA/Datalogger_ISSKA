...
String Sensors::getFileHeader () { // Return the file header for the sensors part. This should be something like "<sensor 1 name>;<sensor 2 name>;"
  return "tempSHT;humSHT;tempBMP;pressBMP;tempEAU;pressEAU;htWat;"; //change if you add sensors !!!!!!!!
}
...
String Sensors::getFileData () { // Return sensors data formated to be write to the CSV file. This should be something like "<sensor 1 value>;<sensor 2 value>;"
  ...
  sprintf(str, "%s;%s;%s;%s;%s;%s;%s;", //"temp", "press", "Hum" , ... put as many %s; as there are sensors!!!!!! don't forget the last ";" !!
     String(mesures.tempSHT,1).c_str(), String(mesures.humSHT,1).c_str(), String(mesures.tempBMP,3).c_str(),String(mesures.pressBMP,6).c_str(), String(mesures.tempWAT,4).c_str(), String(mesures.presWAT,7).c_str(),String(mesures.htWat,7).c_str());
  return str;
}
...
String Sensors::serialPrint() { //Display sensor mesures to Serial for debug purposes
  ...
  sensor_values_str = sensor_values_str + "t_EAU: " + String(mesures.tempWAT,2) + "\n";
  sensor_values_str = sensor_values_str + "P_EAU: " + String(mesures.presWAT,5) + "\n";
...
//connect and start the SHT35 PCB sensor 
  ...
  //measure pressure of Keller sensor
    ...
    mesures.presWAT=waterPressureBar;
    mesures.htWat=waterDepthM;
    mesures.tempWAT=waterTempertureC;
}
