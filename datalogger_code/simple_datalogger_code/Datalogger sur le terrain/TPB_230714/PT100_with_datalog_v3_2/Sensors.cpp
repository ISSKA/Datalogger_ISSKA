...
String Sensors::getFileHeader () { // Return the file header for the sensors part. This should be something like "<sensor 1 name>;<sensor 2 name>;"
  return (String)"tempSHT;press;hum;tempBMP;tempPT100;"; //change if you add sensors !!!!!!!!
}
...
String Sensors::getFileData () { // Return sensors data formated to be write to the CSV file. This should be something like "<sensor 1 value>;<sensor 2 value>;"
  char str[200];
  sprintf(str, "%s;%s;%s;%s;%s;", //"temp", "press", "Hum" , ... put as many %s; as there are sensors!!!!!! don't forget the last ";" !!
     String(mesures.tempSHT).c_str(), String(mesures.press).c_str(), String(mesures.hum).c_str(),String(mesures.tempBMP).c_str(),String(mesures.tempPT100).c_str());
  return str;
}
...
String Sensors::serialPrint() { //Display sensor mesures to Serial for debug purposes
  ...
  sensor_values_str = sensor_values_str + "press: " + String(mesures.press,0) + "\n";
  ...
  sensor_values_str = sensor_values_str + "PT100: " + String(mesures.tempPT100) + "\n";
...
void Sensors::mesure() {
  ...
  delay(40);
  mesures.tempPT100 = PT100_sensor.readPT100Centigrade();
  delay(40);
