/* *******************************************************************************
Code for datalogger TPB_230328 with Notecard, 2 MS8067 (temp, hum, press), 1 SCD41 CO2 sensor and an O2 sensor
...
bool connect_notecard(){ //returns true if it was able to connect the notecard to GSM nework
  ...
  while(hubStatus_str != "connected (session open) {connected}" && !GSM_time_out){
    ...
    if((current_time-start_time_connection)/1000000>240){ //attendre au max 4 min
      GSM_time_out = true;
      digitalWrite(MOSFET_NOTECARD_PIN, LOW);
      return false;
    }
...
void send_data_overGSM(){ //send several measurements all at once over GSM
  ...
  */
  ...
  if(connect_notecard()){ //connect the notecard
    ...
    String sensor_names[num_sensors+1] = {"VBat","tempEXT","pressEXT","humEXT","tempSOIL","pressSOIL","humSOIL","CO2","tempCO2","humCO2","voltageO2","percentageO2"}; //change this if you add more sensors!!!
    ...
    while(needToWait==true && !GSM_time_out){
      ...
      unsigned long current_time = micros();
      if((current_time-start_time_connection)/1000000>300){ //attendre au max 5 min
        GSM_time_out = true;
        digitalWrite(MOSFET_NOTECARD_PIN, LOW);
