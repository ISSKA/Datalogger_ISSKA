...
String names[]={"Vbatt","tempSHT","humSHT","tempBMP","pressBMP","O2","CO2","tempCO2","humCO2"}; //update this list if you add sensors!!!
...
void Sensors::measure() {
  ...
  for(int i = 0; i < 5; i ++){
    O2sensor.send_read_with_temp_comp(values[3]);
    delay(1000);
  ...
  delay(1000);
  O2sensor.receive_read_cmd();              //get the response data and put it into the [Device].reading variable if successful
  values[5] = O2sensor.get_last_received_reading(); //print either the reading or an error message
  delay(400);
  //CO2 sensor values
  tcaselect(7);
  delay(100);
  ...
  values[6]=co2_sensor.getCO2();
  values[7]=co2_sensor.getTemperature();
  values[8]=co2_sensor.getHumidity();
  delay(100);
}
