...
String Sensors::getFileHeader () {
  return "temperatureBox[degC];pressureBox[mbar];humidityBox[RH%];Temp_BMP[degC];Pressure_BMP[Pa]";
}
...
String Sensors::getFileData () {
  ...
  sprintf(str, "%s;%s;%s;%s;%s",
     String(mesures.tempBox, 3).c_str(), String(mesures.pressBox, 3).c_str(), String(mesures.humidityBox, 3).c_str(), String(mesures.tempBMP, 3).c_str(), String(mesures.pressBMP, 3).c_str());
