...
void setup() {
  ...
  pinMode(MOSFET_PIN, OUTPUT);         // RTC and status LED power via pin D7 (about 3 mA)
  ...
  readingID ++;
  if (readingID == 1){
    deep_sleep_mode(wakeUpInterval - 140); //22 is the time shift for the program execution
  }
  else {
    deep_sleep_mode(wakeUpInterval - 125); //2 is the time shift for the program execution
  }
}
