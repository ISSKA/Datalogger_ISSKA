...
String names[]={"Vbatt","tempSHT","humSHT","tempBMP","pressBMP","waterdepth [mm]"}; //update this list if you add sensors!!!
...
//get water height
  ...
  //turn relay on
  if(!relay.begin()){
    Serial.println(F("I2C communication with Relay failed !"));
  }
  delay(100);
  relay.turnRelayOn();
  ...
  float ADC_count = ads1115.readADC_SingleEnded(3);//Sensors output is connected to A0 input
  Serial.println(F("A3 voltage measure"));
  float voltageDFrobotmV = ads1115.computeVolts(ADC_count) * 1000; //*1000 to convert V to mV
  float dataCurrent = voltageDFrobotmV / 120.0; //Sense Resistor:120ohm (in mA)
  values[5]= (dataCurrent - CURRENT_INIT) * (RANGE / 16.0)*0.1; //Calculate depth from current readings (in cm)
  delay(100);
  relay.turnRelayOff();
