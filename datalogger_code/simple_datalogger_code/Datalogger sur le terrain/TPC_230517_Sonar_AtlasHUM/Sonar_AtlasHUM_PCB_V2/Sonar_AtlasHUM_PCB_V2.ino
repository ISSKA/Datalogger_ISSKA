...
void setup(){ //The setup is recalled each time the micrcontroller wakes up from a deepsleep
  ...
  Wire.setClock(150000); // set I2C clock frequency to 50kHz which is rather slow but more reliable //change to lower value if cables get long
  ...
  if(first_time){ //sleep until rounded time, this snipet is only executed when the microcontroller is turned on for the first time
    ...
    u8x8.begin();
    u8x8.setBusClock(150000); //Set the SCL at 50kHz since we can have long cables in some cases. Otherwise, the OLED set the frequency at 500kHz
...
void power_external_device(){
  //Power the 5V converter
  pinMode(Enable5VPIN, OUTPUT);    
  digitalWrite(Enable5VPIN, HIGH);  // Switch 5V converter on
