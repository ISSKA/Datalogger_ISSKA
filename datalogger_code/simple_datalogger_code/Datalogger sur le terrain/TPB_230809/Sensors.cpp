...
String Sensors::getFileData () { // Return sensors data formated to be write to the CSV file. This should be something like "<sensor 1 value>;<sensor 2 value>;"
  ...
  sprintf(str, "%s;%s;%s;%s;%s;%s;%s;%s;", //"temp", "press", "Hum" , ... put as many %s; as there are sensors!!!!!! don't forget the last ";" !!
     String(mesures.tempSHT,1).c_str(), String(mesures.humSHT,1).c_str(), String(mesures.tempBMP).c_str(),String(mesures.pressBMP).c_str(), String(mesures.tempPluv,3).c_str(), String(mesures.presPluv,3).c_str(),String(mesures.presWat,3).c_str(),String(mesures.htWat,3).c_str());
  return str;
}
...
String Sensors::serialPrint() { //Display sensor mesures to Serial for debug purposes
  ...
  sensor_values_str = sensor_values_str + "T_BMP: " + String(mesures.tempBMP) + "\n";
  sensor_values_str = sensor_values_str + "P_BMP: " + String(mesures.pressBMP) + "\n";
