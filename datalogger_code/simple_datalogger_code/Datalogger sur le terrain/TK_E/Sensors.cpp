...
String names[]={"Vbatt","tempSHT","humSHT","tempBMP","pressBMP","Flow1[L/min]","FlowSPD1[m/s]",}; //update this list if you add sensors!!!
...
String Sensors::getFileData () { 
  String datastring = "";
  for(int i = 0; i< nb_values; i++){
    datastring = datastring + String(values[i],7) + ";";
  }
  return datastring;
}
...
void Sensors::measure() {
  ...
  delay(100);
  tcaselect(7);
  sfmSf06_1.begin(Wire, ADDR_SFM3003_300_CE);
  delay(100);
...
//  delay(100);
  ...
  delay(100);
  sfmSf06_1.readMeasurementData(flow, temperature, status);
  delay(1000);
  values[5]=(flow+12288)/120;
    //SFMmoyenne +=flowcorr;

  float vent1=values[5];
	  if (vent1>0){
      values[6]=(0.0003*pow(vent1,3))-(0.0125*pow(vent1,2))+(0.274*(vent1));
  }
  else{
      values[6]=(-0.0003*pow(vent1,3))-(0.0125*pow(vent1,2))+(0.274*(vent1));
