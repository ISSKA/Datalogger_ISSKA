...
String names[]={"Vbatt","tempSHT","humSHT","tempBMP","pressBMP","tempMS5803","pressMS5803","waterlevel"}; //update this list if you add sensors!!!
...
void Sensors::measure() {
  ...
  //Part that give instruction to the pump

  Serial2.print("DC,20,1");    //Pump instructions that can be changed according to the need (see https://files.atlas-scientific.com/EZO_PMP_Datasheet.pdf)
  Serial2.print('\r');
  ...
  sensor1.begin();
  delay(1000);
  values[5] = sensor1.getTemperature(CELSIUS, ADC_256);
  values[6] = sensor1.getPressure(ADC_256);

  float p1 = values[4];
  float p2 = values[7];
  values[7] = (p2-p1)*1.01972;
  delay(300);
  tcaselect(1);
  delay(1000);
}
