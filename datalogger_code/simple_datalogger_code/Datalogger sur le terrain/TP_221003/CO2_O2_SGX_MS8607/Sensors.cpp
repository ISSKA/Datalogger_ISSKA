...
String Sensors::getFileHeader () {
  return "temperature;pressure;humidityBOX;CO2;O2;voltageO2;tempCO2;humiCO2";
}
...
String Sensors::getFileData () {
  ...
  sprintf(str, "%s;%s;%s;%s;%s;%s;%s",
          String(mesures.tempBox, 2).c_str(), String(mesures.pressBox, 2).c_str(), String(mesures.humidityBox, 3).c_str(), String(mesures.co2).c_str(), String(mesures.o2, 2).c_str(), String(mesures.voltage, 5).c_str(), String(mesures.humidityCO2, 2).c_str());
...
void Sensors::mesure() {
  ...
  float O2_voltage_tcomp=0.002864*mesures.tempBox+1.124689;
  Serial.print("Voltage: ");Serial.println(voltage);
  //O2 sensor is the SGX-40X
  //0% -> 0V    18.4% -> 1.2V
  //Mettre un meilleur calcul ici + compensation de la température
  mesures.o2 = (voltage*0.184/O2_voltage_tcomp)*100;
  mesures.voltage=voltage;
}
