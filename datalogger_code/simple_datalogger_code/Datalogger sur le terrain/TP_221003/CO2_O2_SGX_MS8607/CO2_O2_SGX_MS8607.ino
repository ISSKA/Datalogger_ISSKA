...
void setup() {
  ...
  //RTC.adjust(DateTime(2022, 03, 30, 11, 50, 0));
  ...
  readingID ++;
  if (readingID == 1){
    deep_sleep_mode(wakeUpInterval-55); //28 is the time shift for the program execution
  }
  else {
    deep_sleep_mode(wakeUpInterval-22); //2 is the time shift for the program execution
  }
}
...
void displayToOled(float batvolt) {
  // write to display
  Serial.println(F("display to oled"));
  ...
  sleep(4);
  Serial.println(F("display sensors data"));
...
boolean writeFileHeader() {
  Serial.println(F("file header write"));
  ...
  Serial.println(F("logFile available"));
...
void deep_sleep_mode(uint32_t nbSeconds){
...
//  btStop();
  ...
  adc_power_release();
    ...
    RTC.disableAlarm(2);              // turn off alarm 2 (in case it isn't off already)
     //28 is the time shift for the program execution
    nbSeconds=nbSeconds-2;
   TimeSpan ts=nbSeconds;
//   Serial.println(ts.totalseconds());
//   Serial.println(ts.seconds());
//   Serial.println(ts.minutes());
   //ts.totalseconds=nbSeconds;
   DateTime dt=RTC.now();
   uint32_t timenow = dt.unixtime();
   RTC.setAlarm1(timenow+nbSeconds,DS3231_A1_Minute);            // this mode triggers the alarm when the seconds match
        
        
//   Serial.print(F("nbSeconds is: "));
//   Serial.println(nbSeconds);
//   Serial.print(F("wakeupinterval is: "));
