...
void setup() {
  ...
  //write header in SD
    ...
    logFile.println();
    logFile.print("ID;DateTime;VBatt;AirSpeed_TOP;\n");
    logFile.close();
...
void loop() {
  ...
  delay(100);
  float speed1=air_velocity_sensor1.readMetersPerSecond();
  delay(100);
  tcaselect(6);
  air_velocity_sensor2.setRange(AIRFLOW_RANGE_7_MPS);
  delay(100);
  float speed2=air_velocity_sensor2.readMetersPerSecond();
  delay(100);
  ...
  u8x8.clear();
  u8x8.setCursor(0,0);
  u8x8.print(String(speed1,3));
  u8x8.setCursor(0, 4);
  u8x8.print(String("m/s"));
