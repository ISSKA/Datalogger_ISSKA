...
#define SD_CS_PIN 34//Define CS pin for the SD card module
...
void setup() {
  ...
  tiS3.begin();
  tiS3.setPixelPower(true); //This method turns on the Dotstar power switch, which enables the tinypico LED
  tiS3.setPixelColor(25, 25, 25 ); //Turn the LED white while SD card is not initialised
 ...
 // digitalWrite(MOSFET_SENSORS_PIN, HIGH);
  ...
  //write header in SD
    ...
    logFile.println();
    logFile.print("ID;DateTime;VBatt;v_mA;\n");
    logFile.close();
  ...
  float batvolt = tiS3.getBatteryVoltage();
...
void loop() {
 ...
 //u8x8.setFont(u8x8_font_courB24_3x4_r);
  ...
  u8x8.setCursor(0,2);
  u8x8.print(String(current_mA,2));
  u8x8.setCursor(0,4);
  u8x8.print("Voltage:");
  u8x8.setCursor(0,6);
  u8x8.print(String(loadvoltage,2));
