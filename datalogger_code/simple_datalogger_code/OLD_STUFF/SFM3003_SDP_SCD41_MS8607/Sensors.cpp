...
String Sensors::getFileHeader () {
  return "temperature;pressure;Box_humidity;P_SDP;debitSDP;velocitySFM;debitSFM;CO2";
}
...
String Sensors::getFileData () {
  ...
  sprintf(str, "%s;%s;%s;%s;%s;%s;%s;%s",
     String(mesures.temp1,3).c_str(), String(mesures.press1,2).c_str(), String(mesures.humidity1,2).c_str(),String(mesures.P_SDP, 7).c_str(),String(mesures.Q_SDP, 7).c_str(), String(mesures.velSFM, 7).c_str(),String(mesures.debitSFM, 5).c_str(), String(mesures.co2).c_str());
