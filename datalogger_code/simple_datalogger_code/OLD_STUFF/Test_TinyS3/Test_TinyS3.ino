...
//Code version du 20.07.2022 pour la carte TinyS3
...
#define MOSFET_PIN 2
// Define CS pin for the SD card module (CS = chip select equivalent to SS = Slave select)
#define SD_CS 34
...
void setup() {
  ...
  tinyS3.setPixelColor( 44, 65, 203 );
  sleep(2);
  if (readingID == 0) {
    firstRun = true;
  }
  ...
  if (firstRun || ALWAYS_DISPLAY_TO_OLED) {
    ...
    u8x8.setBusClock(I2C_FREQUENCY);
    u8x8.begin();
  ...
  if (firstRun || ALWAYS_DISPLAY_TO_OLED) {
    ...
    tinyS3.setPixelColor( 0, 255, 0 );
    sleep(2);
    tinyS3.setPixelPower(false);
  }
  ...
  RTC.disable32K();
  batvolt = tinyS3.getBatteryVoltage();
  ...
  readingID ++;
  if (readingID == 1){
    deep_sleep_mode(wakeUpInterval - 48); //28 is the time shift for the program execution
  }
  else {
    deep_sleep_mode(wakeUpInterval - 15); //2 is the time shift for the program execution
  }
}
